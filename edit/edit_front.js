...
function saveImgLeft() {
	if (page != 0) {
		...
		var lastPhoto = "/show/000001_000001_page0.png"; //showURL(userId, book_id, page).split("/raw")[1];
		deleteFile({
			accountId: "kW15c5s",
			...
			() => {console.log("Success.");
				uploadFile(newState).then(
					(res) => {console.log("Success."); 
						//updateFunc(userId, book_id, page, res);
						page -= 1;
						refresh();
					},
...
function saveImgLeave() {
	...
	for (var i=0; i<totalPage; i++) {
		var a = 1; //ifFirst(userId, book_id, i);
		if (a != 0) {
			complete += 1;
		}
	}
...
$("#shareBtn").click(function () {
	...
	base64Images = [];
	var havePage = 2; //havePageFunc(userId, book_id);
...
function createBase64 (n) {
	...
	async function loadImg () {
		...
		var allImg = ["https://upcdn.io/kW15c5s/raw/bookpage/000007-page0.png", "https://upcdn.io/kW15c5s/raw/bookpage/transparent.png"]; //shareURL(pageNow)
